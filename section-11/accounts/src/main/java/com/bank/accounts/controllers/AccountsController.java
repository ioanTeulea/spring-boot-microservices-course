...
public class AccountsController {
    ...
    @Retry(name = "accountsRetry", fallbackMethod = "getBuildVersionFallback")
    @GetMapping(path="/version")
    public ResponseEntity<String> getBuildVersion() {
        logger.debug("Fetching build version information");
        throw new NullPointerException();
      //return ResponseEntity.status(HttpStatus.OK).body(buildVersion);
    }
    public ResponseEntity<String> getBuildVersionFallback(Throwable throwable) {
        logger.debug("getBuildVersionFallback called due to: {}", throwable.toString());
    ...
    @RateLimiter(name="accountsRateLimiter", fallbackMethod = "getJavaVersionFallback")
    @GetMapping(path="/java-version")
    public ResponseEntity<String> getJavaVersion() {
        return ResponseEntity.status(HttpStatus.OK).body(environment.getProperty("JAVA_HOME"));
    }
    public ResponseEntity<String> getJavaVersionFallback(Throwable throwable) {
        logger.debug("getJavaVersionFallback called due to: {}", throwable.toString());
        return ResponseEntity.status(HttpStatus.SERVICE_UNAVAILABLE).body("Service is currently unavailable. Please try again later.");
    }
