...
import com.bank.cards.dtos.CardsContactInfoDTO;
...
public class CardsController {
    ...
    public ResponseEntity<CardDTO> fetchCardDetails( @RequestParam @Pattern(regexp = "^\\+?[0-9]{10,15}$", message = "Invalid mobile number format")
        ...
        logger.debug("Received request to fetch card details for mobile number: {} with correlation ID: {}", mobileNumber, correlationId);
    ...
    @Operation(summary = "Get build information", description = "Endpoint to get cards service build version information")
    @ApiResponses({
            @ApiResponse( responseCode = "200", description = "HTTP Status OK"),
            @ApiResponse(responseCode = "500", description = "HTTP Status Internal Server Error",content = @Content(schema = @Schema(implementation = ErrorResponseDTO.class)))
    })
    ...
    @Operation(summary = "Get java version", description = "Endpoint to get cards service java build version information")
    @ApiResponses({
            @ApiResponse( responseCode = "200", description = "HTTP Status OK"),
            @ApiResponse(responseCode = "500", description = "HTTP Status Internal Server Error",content = @Content(schema = @Schema(implementation = ErrorResponseDTO.class)))
    })
    ...
    @Operation(summary = "Get contact info", description = "Endpoint to get card contact information")
    @ApiResponses({
            @ApiResponse( responseCode = "200", description = "HTTP Status OK"),
            @ApiResponse(responseCode = "500", description = "HTTP Status Internal Server Error",content = @Content(schema = @Schema(implementation = ErrorResponseDTO.class)))
    })
