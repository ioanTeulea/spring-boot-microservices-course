...
public class RequestTraceFilter implements GlobalFilter {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        if(isCorrelationIdPresent(requestHeaders)){
            logger.debug("Correlation id found in the request headers: {}. ", filterUtility.getCorrelationId(requestHeaders));
        }
        else{
            String correlationId = generateCorrelationId();
            exchange = filterUtility.setRequestHeader(exchange, correlationId);
            logger.debug("Correlation id generated and set in the request headers: {}. ", correlationId);
        }
